...
QUESTIONS (gaps) — name what only the CANDIDATE can supply that would most RAISE those scores: e.g. missing metrics for an achievement, the target job title/industry to tune keywords toward, a claimed skill with no evidence, missing contact details, an unexplained gap. Ask each as a direct question they can answer in a sentence or two. Whenever an achievement is unquantified, ASK FOR THE NUMBER and show the shape of the answer in the question — a percentage, count, time saved, money or scale (e.g. "By roughly what percentage did you cut infrastructure costs? — e.g. ~15%"). Never put an invented figure on the CV yourself; only the candidate's real answer (folded in on the next pass) may add a number. Give 2 to 5 questions, ordered by how much they'd help; skip anything already well answered. (These overlap with "issues" but are phrased as answerable questions for a follow-up pass.)
